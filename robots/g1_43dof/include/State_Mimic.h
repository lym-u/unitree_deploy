...
public:
  ...
  void load_data_from_npz(const std::string &motion_file) {
    ...
    auto kp = npz_data["joint_pos"];
    auto kd = npz_data["joint_vel"];
    if (need_set_kpkd_) {
      kp = npz_data["joint_kp"]; // [frame, dof]
      kd = npz_data["joint_kd"]; // [frame, dof]
    }
    ...
    for (size_t i = 0; i < num_frames_npz; i++) {
      ...
      const size_t body_stride_quat =
          body_quat_w.shape[1] * body_quat_w.shape[2];
      ...
      if (need_set_kpkd_) {
        Eigen::VectorXf kp_value(kp.shape[1]);
        for (int j = 0; j < kp.shape[1]; j++) {
          kp_value[j] = kp.data<float>()[i * kp.shape[1] + j];
        }

        Eigen::VectorXf kd_value(kd.shape[1]);
        for (int j = 0; j < kd.shape[1]; j++) {
          kd_value[j] = kd.data<float>()[i * kd.shape[1] + j];
        }
        dof_kps.push_back(kp_value);
        dof_kds.push_back(kd_value);
      }
    }
  }
  ...
  void reset(const isaaclab::ArticulationData &data, float t = 0.0f) {
    update(t);
    auto init_to_anchor =
        isaaclab::yawQuaternion(this->root_quaternion()).toRotationMatrix();
    auto world_to_anchor =
        isaaclab::yawQuaternion(data.root_quat_w).toRotationMatrix();
    world_to_init_ = world_to_anchor * init_to_anchor.transpose();
  }

  Eigen::VectorXf root_position() { return root_positions[frame]; }
  Eigen::Quaternionf root_quaternion() { return root_quaternions[frame]; }
  Eigen::VectorXf joint_pos() { return dof_positions[frame]; }
  Eigen::VectorXf joint_vel() { return dof_velocities[frame]; }
  Eigen::VectorXf joint_kp() { return dof_kps[frame]; }
  Eigen::VectorXf joint_kd() { return dof_kds[frame]; }
  ...
  std::vector<Eigen::VectorXf> dof_kps;
  std::vector<Eigen::VectorXf> dof_kds;
