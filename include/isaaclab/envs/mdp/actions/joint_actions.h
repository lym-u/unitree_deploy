...
public:
  JointAction(YAML::Node cfg, ManagerBasedRLEnv *env) : ActionTerm(cfg, env) {
  ...
  virtual void process_actions(std::vector<float> actions) {
    ...
    if (!_clip.empty()) {
      for (int i(0); i < _action_dim; ++i) {
        _processed_actions[i] =
            std::clamp(_processed_actions[i], _clip[i][0], _clip[i][1]);
      }
    }
  }

  int action_dim() { return _action_dim; }

  std::vector<float> raw_actions() { return _raw_actions; }

  std::vector<float> processed_actions() { return _processed_actions; }

  void reset() { process_actions(std::vector<float>(_action_dim, 0.0f)); }
